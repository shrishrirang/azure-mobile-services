...
namespace Microsoft.WindowsAzure.MobileServices
{
    ...
    internal sealed class PushHttpClient
    {
        ...
        {
            var response = await this.client.MobileAppCodeSiteClient.RequestAsync(HttpMethod.Get, string.Format("/push/registrations?deviceId={0}&platform={1}", Uri.EscapeUriString(deviceId), Uri.EscapeUriString(Platform.Instance.PushUtility.GetPlatform())), this.client.CurrentUser);
        ...
        {
            return this.client.MobileAppCodeSiteClient.RequestAsync(HttpMethod.Delete, string.Format("/push/registrations/{0}", Uri.EscapeUriString(registrationId)), this.client.CurrentUser, ensureResponseContent: false);
        }

        public async Task<string> CreateRegistrationIdAsync()
        {
            var response = await this.client.MobileAppCodeSiteClient.RequestAsync(HttpMethod.Post, "/push/registrationids", this.client.CurrentUser, null, null);
        ...
        public async Task CreateOrUpdateRegistrationAsync(Registration registration)
        {
            ...
            await this.client.MobileAppCodeSiteClient.RequestAsync(HttpMethod.Put, string.Format("/push/registrations/{0}", Uri.EscapeUriString(regId)), this.client.CurrentUser, content, ensureResponseContent: false);
